...
import { LayoutGrid, TrendingUp, FileText, MessageSquare, MessageCircle, Calendar, User } from 'lucide-react';
...
export const BottomNavBar: React.FC<BottomNavBarProps> = ({ currentTab, onTabChange }) => {
  const navItems: { tab: NavigationTab; label: string; icon: React.ReactNode; activeIcon: React.ReactNode }[] = [
    ...
    },
    {
      tab: 'messaging',
      label: '메시지',
      icon: <MessageCircle className="w-5 h-5 mb-1 text-[#434654]" />,
      activeIcon: <MessageCircle className="w-5 h-5 mb-1 text-[#003fb1] stroke-[2.5]" />
    },
    {
      tab: 'calendar',
      label: '일정',
      icon: <Calendar className="w-5 h-5 mb-1 text-[#434654]" />,
      activeIcon: <Calendar className="w-5 h-5 mb-1 text-[#003fb1] stroke-[2.5]" />
    },
    {
      tab: 'mypage',
      label: '마이',
  ...
  return (
    <nav className="bg-[#f9f9ff] fixed bottom-0 left-0 w-full z-40 flex justify-around items-center h-16 pb-safe px-2 border-t border-[#c3c5d7]/50 md:hidden overflow-x-auto scrollbar-none backdrop-blur-md bg-opacity-95">
      {navItems.map((item) => {
        ...
        return (
          <button
            ...
            className={`flex flex-col items-center justify-center flex-1 h-full rounded-xl transition-all duration-150 cursor-pointer min-w-[60px] ${
              isActive
